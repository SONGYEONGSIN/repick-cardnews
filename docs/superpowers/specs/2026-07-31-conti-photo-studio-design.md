...
| # | 스텝 | 화면 | 인터랙션 |
|---|---|---|---|
| 1 | 사진 | 대형 드롭존(폴더 통째로 드롭) + `webkitdirectory` 폴더 선택 버튼 → 썸네일 그리드. 파일명·해상도·용량·4:5 크롭 경고 배지 | 드롭 · 폴더 선택 · 개별 제외 토글 |
| 2 | 순서 | 드래그앤드롭 리오더 레일 + 미사용 트레이. 정원 초과분은 트레이로 내려간다 | **dnd 리오더(키보드 포함)** |
| 3 | 주제 | 키워드 · 유형 · 테마 3종 · 워터마크 핸들 → `카피 생성` | 테마 선택 시 프리뷰 즉시 반영 |
...
```
① 사진    File[] ──photos.ts──▶ Photo[] { dataUrl(원본), thumbUrl(1024px) }
② 순서    dnd-kit ──order.ts──▶ order: photoId[]  /  미사용
③ 주제    keyword · type · themeId · handle
...
요청 바디에 `photos: string[]`(dataURL, 선택)이 추가된다. 라우트는 각 dataURL에서 media type과 base64 본문을 갈라 Anthropic `image` 블록으로 만들고, 텍스트 블록 앞에 순서대로 배치한다. 시스템 프롬프트에 "N번째 사진이 N번째 카드"라는 대응 규칙과 "사진에 보이지 않는 것을 쓰지 말 것"을 추가한다.
...
| `lib/order.test.ts` | reorder(from→to) 결과와 불변성 · 정원 초과분 분리 · 유형별 정원(1 / 5~6) |
...
3. `npm run build` 통과.
4. 폴더 8장을 드롭 → 순서 변경 → 카피 생성 → 편집 → 폴더 저장까지 한 번에 완주하고, `cardnews/<슬러그>-<MMDD>/`에 PNG 5장이 실제로 생성된다.
5. 1280 / 1440 / 1920에서 가로 오버플로 0.
6. 키보드만으로 5스텝 전 경로 도달 — 2번 스텝의 순서 변경 포함.
