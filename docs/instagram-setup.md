...
> 이 절차는 2026-08-02 에 계정 `ys040607` 로 **실제 게시까지 성공한** 경로입니다(카드뉴스
> 캐러셀 5장, PNG). **정보전달 1장 게시도 2026-08-05 에 같은 계정으로 실제 성공**했습니다 —
> 캐러셀과 다른 경로를 쓰므로(`publishSingleImage`) 따로 확인한 것입니다.
> 앱 검수는 필요 없었습니다 — 본인 계정에만 올리는 경우 대시보드가 그 단계를 건너뛰라고 안내합니다.
>
> **예약 발행도 2026-08-05 에 실제 성공했습니다.** 그전까지는 예약이 0바이트 이미지를 저장해
> 인스타그램이 `HTTP 500 · code 1` 로 거절했습니다 — 캡처 결과를 data URL 로 착각해 잘라
> 버린 탓이며, 지금은 서버가 PNG 서명까지 확인해 그런 요청을 아예 받지 않습니다.
...
첫 화면(`/`) → 카드 만들기 → 내보내기 화면 → 인스타 게시 패널에서 **연결 확인** 을 누릅니다.
