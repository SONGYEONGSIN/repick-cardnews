...
describe("runScheduledItem", () => {
  it("이미지가 없으면 게시하지 않고 한국어로 실패한다", async () => {
    ...
    const res = await runScheduledItem(item({ imageUrls: [] }), { now: 1, publish });
  ...
  it("한 장이면 단일 게시 경로로 부른다 — 캐러셀로 보내지 않는다", async () => {
    ...
    const res = await runScheduledItem(item({ imageUrls: ["https://blob.example/scheduled/a1/1.png"] }), {
      now: 1,
      publish,
      publishSingle,
    });
  ...
  it("상한을 넘으면 게시하지 않는다", async () => {
    ...
    const res = await runScheduledItem(item({ imageUrls: Array.from({ length: 11 }, (_, i) => `https://blob.example/scheduled/a1/${i + 1}.png`) }), {
      now: 1,
      publish,
      publishSingle,
    });
  ...
  it("설정이 없으면 무엇이 없는지 한국어로 말한다", async () => {
    ...
    const res = await runScheduledItem(item(), { now: 1, publish });
  ...
  // 예전에는 디스크에서 읽어 **다시 올린 뒤** 그 주소를 넘겼다. Blob 주소는 영구적이라
  // 그럴 이유가 없다 — 예약할 때 올려 둔 주소를 그대로 쓴다.
