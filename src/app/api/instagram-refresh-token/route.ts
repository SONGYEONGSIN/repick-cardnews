 ...
 * 인스타그램 API를 부르지 않고 토큰 값도 어디에도 담기지 않으므로, `/api/instagram-status`와
 * 같은 이유로 `local-guard`를 걸지 않는다 — 화면이 마운트 시 바로 물어볼 수 있어야 한다.
 */
 ...
 * 부르고 성공하면 `.env.local`을 원자적으로 바꾸므로, `/api/publish`·`/api/instagram-verify`와
 * 같은 이유로 이 PC 브라우저에서만 부를 수 있게 막는다(`@/lib/local-guard`, 한계는 그 파일
 * 참고). 24시간 미만이라 거절되는 것도 정상 상황으로 함께 내려준다(`reason: "too-soon"`) —
 * **토큰 값은 어떤 경우에도 응답에 담기지 않는다.**
 */
export async function POST(req: Request) {
  if (!isLocalHost(req.headers.get("host"))) {
    return Response.json({ ok: false, error: "토큰 갱신은 이 컴퓨터의 브라우저에서만 할 수 있어요." }, { status: 403 });
  }
